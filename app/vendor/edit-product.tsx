...
export default function EditProduct() {
  ...
  useEffect(() => {
    if (id) {
      ...
      if (product) {
        // Ensure this product belongs to the current vendor
        const checkOwnership = async () => {
          try {
            ...
            setOrganic(product.organic || false);
            setSku(product.sku || '');
  ...
  useEffect(() => {
    const loadVendorData = async () => {
      try {
        ...
        if (currentVendor) {
          console.log('EditProduct: Using vendor from context:', currentVendor.name);
        ...
        console.log('EditProduct: Loading vendor from AsyncStorage, vendorId:', vendorId);
        
        if (vendorData) {
          const parsedVendor = JSON.parse(vendorData);
          console.log('EditProduct: Loaded vendor from AsyncStorage:', parsedVendor.name);
          setLocalVendor(parsedVendor);
        } else {
          console.error('EditProduct: No vendor data found in AsyncStorage');
        }
      } catch (error) {
  ...
  const handleSubmit = () => {
    ...
    try {
      ...
      updateProduct(productId, {
        ...
        category,
        organic,
        rating: products.find(p => p.id === productId)?.rating || 0,
        description,
            ...
            <View style={styles.formSection}>
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  ...
                  editable={false} // SKU should not be editable once created
                />
              </View>
